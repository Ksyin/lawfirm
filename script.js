...
(function () {
  ...
  const INSIGHT_FALLBACK_IMAGE = "https://images.unsplash.com/photo-1521790797524-b2497295b8a0?auto=format&fit=crop&w=1400&q=80";

  function getPracticeGallery(area) {
    const galleryById = {
      family: [
        "https://images.unsplash.com/photo-1643818657367-491080baeece?auto=format&fit=crop&w=1200&q=80",
        "https://images.unsplash.com/photo-1592599457566-c660153d9548?auto=format&fit=crop&w=1200&q=80",
        "https://images.unsplash.com/photo-1563132337-f159f484226c?auto=format&fit=crop&w=1200&q=80"
      ],
      litigation: [
        "https://images.unsplash.com/photo-1617244147299-5ef406921c35?auto=format&fit=crop&w=1200&q=80",
        "https://images.unsplash.com/photo-1521790797524-b2497295b8a0?auto=format&fit=crop&w=1200&q=80",
        "https://images.unsplash.com/photo-1495603889488-42d1d66e5523?auto=format&fit=crop&w=1200&q=80"
      ],
      conveyancing: [
      ...
      corporate: [
        "https://images.unsplash.com/photo-1495603889488-42d1d66e5523?auto=format&fit=crop&w=1200&q=80",
        "https://images.unsplash.com/photo-1724128195247-ab10e4535b53?auto=format&fit=crop&w=1200&q=80",
        "https://images.unsplash.com/photo-1616805765352-beedbad46b2a?auto=format&fit=crop&w=1200&q=80"
      ],
      employment: [
        "https://images.unsplash.com/photo-1563132337-f159f484226c?auto=format&fit=crop&w=1200&q=80",
        "https://images.unsplash.com/photo-1563132305-24784f57eeea?auto=format&fit=crop&w=1200&q=80",
        "https://images.unsplash.com/photo-1573496130141-209d200cebd8?auto=format&fit=crop&w=1200&q=80"
      ],
      succession: [
        "https://images.unsplash.com/photo-1592599457566-c660153d9548?auto=format&fit=crop&w=1200&q=80",
    ...
    return gallery.filter(Boolean).slice(0, 2);
  }
    ...
    grid.innerHTML = PRACTICE_AREAS.map(
      ...
      <article class="practice-card reveal" id="${area.id}" data-practice-id="${area.id}">
        <div class="practice-card-visual">
          <img src="${area.image || 'https://images.unsplash.com/photo-1521790797524-b2497295b8a0?auto=format&fit=crop&w=1400&q=80'}" alt="${area.title}" loading="lazy">
        </div>
  ...
  function initChatbot() {
    ...
    /* Keep the chatbot panel from ever rising above the header/utility
       ...
       Also keep it from ever rising above the TOP of the visible screen.
       On phones, opening the on-screen keyboard (to type the email or a
       message) shrinks the visible area without changing window.innerHeight
       consistently across browsers — iOS in particular reports this only
       through the visualViewport API, not a "resize" event. Without
       accounting for that, the panel's height stayed fixed at whatever it
       was before the keyboard opened, so on short screens it ended up
       taller than the space actually visible above the keyboard: the panel
       got pushed upward until its header (and the × close button) went off
       the top of the screen, while the bottom (the Start Chat / Send
       button) could end up hidden behind the keyboard. Sizing against the
       live visual-viewport height, with no artificial minimum, fixes both:
       the header and the button are now always within the visible area. */
    const positionChatbotPanel = () => {
      ...
      const viewportCeiling = viewportHeight - topMargin - bottomSafety;
      /* No artificial minimum here on purpose: `available` already reflects
         the real, live gap above the header. Flooring it to a "preferred"
         height (as a previous version of this function did) is exactly
         what let the panel grow taller than the space actually on screen
         and push its header off the top edge. Better to render a more
         compact — but fully visible — panel on very short screens than a
         taller one that hides the close button. */
      const maxHeight = Math.max(0, Math.min(available, viewportCeiling, 560));
    ...
    if (window.visualViewport) {
      window.visualViewport.addEventListener("resize", positionChatbotPanel);
      window.visualViewport.addEventListener("scroll", positionChatbotPanel);
    }
    /* Recheck the moment the email or message field is focused — this is
       exactly when the keyboard opens, and it fires reliably even on the
       browsers/devices where the viewport events above are delayed. */
    [gateEmail, input].forEach((field) => {
      if (!field) return;
      field.addEventListener("focus", () => window.setTimeout(positionChatbotPanel, 300));
    });
    ...
    const ANTHROPIC_API_KEY = "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";

    /* Build a rich system prompt from the site's own data */
    const buildSystemPrompt = () => {
      const practiceList = (typeof PRACTICE_AREAS !== "undefined" ? PRACTICE_AREAS : [])
        .map(a => `- ${a.title}: ${a.summary || ""} ${a.details || ""}`)
        .join("\n");
      const officeList = (typeof OFFICES !== "undefined" ? OFFICES : [])
        .map(o => `${o.name}: ${o.address ? o.address.join(", ") : ""} | Tel: ${o.phones ? o.phones.join(" / ") : ""}`)
        .join("\n");
      const teamList = (typeof TEAM !== "undefined" ? TEAM : [])
        .map(m => `${m.name} — ${m.role}: ${m.focus || ""}`)
        .join("\n");
      return `You are the official AI legal assistant for A.A. Mudanya & Co. Advocates, a Kenyan law firm. Your role is to assist website visitors with general enquiries about the firm's services, guide them to the right practice area, and encourage consultation bookings. You do NOT provide specific legal advice — always clarify that responses are general information only and that a formal consultation with an advocate is required for legal advice.

FIRM NAME: A.A. Mudanya & Co. Advocates
EMAIL: mudanyalawtech.advocates@gmail.com
PHONE: +254 720 847 047
...
IMPORTANT RULES:
- Be professional, warm, and concise. Use British English spelling.
- Never provide specific legal advice — always recommend a consultation.
- If a question is clearly outside the firm's practice areas, politely redirect.
- Keep responses under 120 words unless a detailed explanation is genuinely necessary.
- Always end complex answers by offering to arrange a consultation.
- Never fabricate case outcomes, statistics, or legal information.
- This chat is monitored and transcripts are shared with the firm.`;
    };

    /* Conversation history for multi-turn context */
    const chatHistory = [];

    const replyWithDelay = async (userText) => {
      ...
      try {
        ...
        if (data && data.content && data.content[0] && data.content[0].text) {
          ...
          /* Graceful fallback if API returns unexpected shape */
        ...
        typingBubble.remove();
        /* Network or API error — fall back to keyword engine gracefully */
        const fallback = botReply(userText);
        chatHistory.push({ role: "assistant", content: fallback });
        addMessage(fallback);
      }
    ...
    detailRoot.innerHTML = `
      <div class="container practice-detail-wrap">
        ...
        <div class="practice-detail-grid">
          <div class="practice-detail-copy">
            ...
            <div class="practice-detail-actions">
              <a href="tel:+254720847047" class="btn btn-primary contact-cta">Speak to an Advocate</a>
          ...
          <div class="practice-detail-visual">
            <img src="${area.image || 'https://images.unsplash.com/photo-1521790797524-b2497295b8a0?auto=format&fit=crop&w=1400&q=80'}" alt="${area.title} legal illustration" loading="lazy">
          </div>
        </div>
        ...
        <div class="practice-detail-panel">
          <h3>How We Help <span class="metrics-swipe-hint">Swipe →</span></h3>
  ...
  function renderHeroStats() {
    ...
    wrap.innerHTML = `<div class="container hero-stats-inner">` +
      STATS.map(
        (s) => `
        <div class="stat">
          <span class="stat-value" data-count-target="${s.value}">0</span>
  ...
  /* ---------- mobile nav toggle ---------- */
  /* ---------- shared "Speak to an Advocate" popover (Call / WhatsApp) ---------- */
  function initContactPopover() {
    const PHONE_DISPLAY = "+254 720 847 047";
    const PHONE_TEL = "tel:+254720847047";
    const WHATSAPP_URL = "https://wa.me/254720847047?text=" + encodeURIComponent("Hello, I would like to make a legal enquiry.");

    let popover = document.getElementById("contactPopover");
    if (!popover) {
      popover = document.createElement("div");
      popover.id = "contactPopover";
      popover.className = "contact-popover";
      popover.setAttribute("role", "dialog");
      popover.setAttribute("aria-label", "Contact A.A. Mudanya & Co. Advocates");
      popover.innerHTML = `
        <p class="contact-popover-label">Reach us directly</p>
        <a href="${PHONE_TEL}" class="contact-popover-option contact-popover-call">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.12.9.35 1.79.68 2.63a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.45-1.25a2 2 0 012.11-.45c.84.33 1.73.56 2.63.68A2 2 0 0122 16.92z"/></svg>
          Call ${PHONE_DISPLAY}
        </a>
        <a href="${WHATSAPP_URL}" class="contact-popover-option contact-popover-whatsapp" target="_blank" rel="noopener">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M17.47 14.38c-.3-.15-1.77-.87-2.04-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.17-.17.2-.35.22-.65.07-.3-.15-1.24-.46-2.37-1.46-.88-.78-1.47-1.75-1.64-2.05-.17-.3-.02-.46.13-.61.13-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.07-.15-.67-1.6-.92-2.2-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.79.37-.27.3-1.04 1.02-1.04 2.48 0 1.46 1.07 2.87 1.22 3.07.15.2 2.1 3.2 5.08 4.49.71.31 1.26.49 1.69.62.71.23 1.36.2 1.87.12.57-.08 1.77-.72 2.02-1.42.25-.7.25-1.3.17-1.42-.07-.13-.27-.2-.57-.35z"/><path d="M12 2a10 10 0 00-8.5 15.2L2 22l4.9-1.5A10 10 0 1012 2z"/></svg>
          Chat on WhatsApp
        </a>
      `;
    ...
    const positionPopover = (trigger) => {
      ...
      if (top + 140 > window.innerHeight) top = Math.max(12, rect.top - 148);
    ...
    document.addEventListener("click", (e) => {
      const trigger = e.target.closest(".contact-cta");
      if (trigger) {
        e.preventDefault();
  ...
  function initMobileNav() {
    ...
    /* Exclude the "Practice Areas" dropdown toggle here — it must only
       expand/collapse its own submenu, not close the whole mobile menu
       (that would immediately hide the submenu it just opened). Links
       inside the submenu are <a> tags and are still matched below, so
       picking a practice area still closes the full mobile nav. */
    nav.querySelectorAll("a, button:not(.nav-dropdown-toggle)").forEach((link) =>
  ...
  /* ---------- render: client logo strip ---------- */
  function renderClients() {
    const track = document.getElementById("clientsTrack");
    if (!track || typeof CLIENTS === "undefined" || !CLIENTS.length) return;
    const itemsHtml = CLIENTS.map((c) => {
      const inner = c.image
        ? `<img src="${c.image}" alt="${c.name}" loading="lazy"><span>${c.name}</span>`
        : c.name;
      const cls = c.image ? "client-item" : "client-item client-item-text";
      return `<span class="${cls}">${inner}</span><span class="dot">•</span>`;
    }).join("\n      ");
  ...
  function openTeamModal(index) {
    ...
    const areaIsDetailed =
      member.practiceAreas &&
      member.practiceAreas.length &&
      typeof member.practiceAreas[0] === "object";
      ...
      member.practiceAreas && member.practiceAreas.length
        ? areaIsDetailed
          ? `<div class="team-modal-section">
               ...
               <div class="team-modal-areas">${member.practiceAreas
                 .map(
                   (area) =>
                     `<div class="team-modal-area">
                        <h5>${area.title}</h5>
    ...
    const quoteHtml = member.quote
      ? `<div class="team-modal-section team-modal-quote">
           <blockquote>&ldquo;${member.quote}&rdquo;</blockquote>
         </div>`
      : "";

    const valuesHtml =
      member.values && member.values.length
        ? `<div class="team-modal-section">
             <div class="team-modal-tags team-modal-values">${member.values
               .map((v) => `<span>${v}</span>`)
               .join("")}</div>
           </div>`
        : "";
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (isInsightPage) {
      ...
      initScrollReveal();
      setYear();
      window.addEventListener("firebase-content-ready", () => {
        renderInsightDetailPage();
      });
      return;
    }
