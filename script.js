...
(function () {
    ...
    grid.innerHTML = PRACTICE_AREAS.map(
      ...
      <article class="practice-card reveal" id="${area.id}" data-practice-id="${area.id}">
        ...
        <div class="practice-card-body">
          ...
          <h3>${area.title}</h3>
          <p>${area.text}</p>
          <button class="learn-more" type="button" data-practice-id="${area.id}" data-practice="${area.title}">
            Learn More
  ...
  function renderPracticeDetail(areaId) {
    ...
    const area = PRACTICE_AREAS.find((item) => item.id === areaId) || PRACTICE_AREAS[0];
    const detailStats = [
      { label: "What we handle", value: area.bullets[0] },
      { label: "How we assist", value: area.focus },
      { label: "Expected outcome", value: area.details }
    ];
      ...
      <div class="container practice-detail-wrap">
        ...
        <div class="practice-detail-grid">
          <div class="practice-detail-copy">
            ...
            <h2>${area.title}</h2>
            <p class="practice-detail-summary">${area.summary}</p>
            ...
            <div class="practice-detail-actions">
              ...
              <a href="#contact-form" class="btn btn-outline-dark practice-book" data-practice="${area.title}">Book an Appointment</a>
            </div>
          </div>
          <div class="practice-detail-visual">
            <img src="${area.image}" alt="${area.title} legal illustration" loading="lazy">
          </div>
        </div>
  ...
  function renderPracticeDropdown() {
    ...
    menu.innerHTML = PRACTICE_AREAS.map(
      (area) => `<a href="#practiceDetailPage" data-practice-id="${area.id}" data-practice="${area.title}">${area.title}</a>`
    ).join("");
  }
  ...
  function renderFooterPracticeAreas() {
    ...
    list.innerHTML = PRACTICE_AREAS.map(
      (area) => `<li><a href="#practiceDetailPage" data-practice-id="${area.id}" data-practice="${area.title}">${area.title}</a></li>`
    ).join("");
  }
  ...
  function initLearnMoreLinks() {
    document.addEventListener("click", (e) => {
      const trigger = e.target.closest("[data-practice-id]");
      if (trigger) {
        const areaId = trigger.dataset.practiceId;
        if (areaId) {
          renderPracticeDetail(areaId);
          const select = document.getElementById("subject");
          if (select && trigger.dataset.practice) select.value = trigger.dataset.practice;
        }
      }
