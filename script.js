...
(function () {
  ...
  function initChatbot() {
    ...
    const emailChip = document.getElementById("chatbotEmailChip");
    const emailForm = document.getElementById("chatbotEmailForm");
    const emailInput = document.getElementById("chatbotEmailInput");
    ...
    const botReply = (message) => {
      ...
      if (!text.trim()) return "Please provide a few further details regarding your matter.";
      if (text.includes("hello") || text.includes("hi")) return "Good day. Thank you for contacting A.A. Mudanya & Co. Advocates. Please describe the legal matter you wish to raise.";
      if (text.includes("consult") || text.includes("appointment") || text.includes("book")) return "A consultation may be arranged by calling +254 720 847 047 or by submitting your details through the contact form on this site. Our team will confirm a suitable time.";
      if (text.includes("land") || text.includes("property") || text.includes("title") || text.includes("lease")) return "This falls within our Conveyancing & Real Estate practice. We advise on title review, land transfers, lease agreements and related due diligence.";
      if (text.includes("family") || text.includes("divorce") || text.includes("custody") || text.includes("children")) return "This falls within our Family Law practice. Such matters are handled with the appropriate care, discretion and confidentiality.";
      if (text.includes("business") || text.includes("company") || text.includes("contract") || text.includes("commercial")) return "This falls within our Corporate & Commercial practice, covering contracts, company governance, compliance and business transactions.";
      if (text.includes("court") || text.includes("litigation") || text.includes("dispute")) return "This falls within our Litigation & Dispute Resolution practice. We represent clients in negotiation, arbitration and court proceedings.";
      if (text.includes("employment") || text.includes("labour") || text.includes("termination") || text.includes("redundancy")) return "This falls within our Employment & Labour Law practice, including contracts, disciplinary matters, redundancy and termination.";
      if (text.includes("data") || text.includes("privacy") || text.includes("gdpr")) return "This falls within our Data Protection practice, covering compliance, breach response and regulatory matters.";
      if (text.includes("estate") || text.includes("succession") || text.includes("probate") || text.includes("will")) return "This falls within our Succession & Probate practice, covering wills, estate administration and related disputes.";
      if (text.includes("thank")) return "You are welcome. Please let us know if there is anything further we can assist with.";
      return "Thank you for the information provided. A member of our legal team will be better placed to advise further — you may share your email above and we will forward this enquiry directly to the firm.";
    };
    ...
    toggle.addEventListener("click", () => {
      ...
      if (!panel.hidden) { positionChatbotPanel(); input.focus(); }
    ...
    if (emailChip && emailForm && emailInput) {
      emailChip.addEventListener("click", () => {
        emailForm.hidden = false;
        emailInput.focus();
        addMessage("Please enter your email address below and I will forward this conversation to our legal team.");
      });
      ...
      emailForm.addEventListener("submit", (e) => {
        ...
        const transcript = Array.from(messages.querySelectorAll(".chatbot-message"))
        ...
        fetch("https://formsubmit.co/ajax/mudanyalawtech.advocates@gmail.com", {
          method: "POST",
          headers: { "Content-Type": "application/json", Accept: "application/json" },
          body: JSON.stringify({
            _subject: "New website chatbot enquiry",
            email: email,
            message: transcript
          })
        })
          ...
          .then(() => {
            addMessage("Thank you. Your enquiry has been sent to our team and we will contact you shortly.");
            emailForm.hidden = true;
            emailForm.reset();
          })
          .catch(() => {
            addMessage("We were unable to send this automatically. Please call +254 720 847 047 or email mudanyalawtech.advocates@gmail.com directly.");
          })
          .finally(() => {
  ...
  function renderInsightDetailPage() {
    ...
    const others = INSIGHTS.filter((item) => item.id !== post.id);
    ...
    root.innerHTML = `
      <section class="practice-detail-page practice-detail-page--full active">
        <div class="container practice-detail-wrap">
          ...
          <div class="practice-detail-grid">
            <div class="practice-detail-copy">
              <p class="section-tag">${post.tag}</p>
              <h2>${post.title}</h2>
            ...
            <div class="practice-detail-visual">
              <img src="${post.image || 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=1400&q=80'}" alt="${post.title}" loading="lazy">
            </div>
          </div>
          ${others.length ? `
          <div class="practice-detail-panel">
            ...
            <ul class="insight-related-list">
              ${others.map((item) => `<li><a href="${buildInsightUrl(item.id)}">${item.title}</a></li>`).join("")}
            </ul>
          </div>` : ""}
        </div>
      </section>
