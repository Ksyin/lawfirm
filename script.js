...
(function () {
  ...
  function initChatbot() {
    ...
    const botReply = (message) => {
      ...
      // greetings
      if (/(^|\s)(hello|hi|hey|habari|mambo)(\s|$)/i.test(text)) return "Hello! We are here to help — would you like to book a consultation, speak to an advocate, or ask about a specific service?";

      // booking and contact intents -> instruct to open modal
      if (text.includes("book") || text.includes("appointment") || text.includes("consultation")) {
        return { type: "openModal", practice: "" };
      }

      // speak to advocate / call / phone
      if (text.includes("call") || text.includes("phone") || text.includes("speak to an advocate") || text.includes("speak")) {
        return { type: "call", phone: FIRM.phone || "+254720847047" };
      }

      // WhatsApp requests
      if (text.includes("whatsapp") || text.includes("wa.me") || text.includes("chat on whatsapp")) {
        return { type: "openUrl", url: `https://wa.me/${FIRM.whatsapp}` };
      }

      // practice-specific keywords
      if (/(land|property|title|conveyanc)/.test(text)) return "That sounds like a conveyancing and real estate matter. We can assist with title review, land transfers, leases and due diligence.";
      if (/(divorce|custody|maintenance|family|children)/.test(text)) return "We handle family law matters (divorce, custody, maintenance) with care and confidentiality. Would you like to book an appointment?";
      if (/(contract|company|commercial|shareholder|corporate)/.test(text)) return "Corporate and commercial law: contracts, company formation, governance and compliance. We provide practical commercial advice tuned to your business needs.";
      if (/(employment|labour|termination|redundancy|discipline)/.test(text)) return "Employment law: we advise on contracts, disciplinary procedures, redundancies and disputes. We can represent employers or employees.";
      if (/(will|probate|succession|estate)/.test(text)) return "Succession & probate: wills, estate administration and beneficiary matters. We can prepare wills and guide executors through probate.";
      if (/(debt|debt recovery|garnishee|collection)/.test(text)) return "Debt recovery: negotiation, demand letters and enforcement where necessary to recover owed sums while protecting commercial relationships where possible.";
      if (/(intellectual|trademark|copyright|ip|patent)/.test(text)) return "Intellectual property: we advise on trademarks, copyrights, registration and licensing to protect your creative or commercial rights.";
      if (/(arbitration|mediation|adr)/.test(text)) return "Arbitration & ADR: private, confidential dispute resolution options that may save time and costs compared to court proceedings.";

      // document and process requests
      if (text.includes("documents") || text.includes("what to bring") || text.includes("checklist")) return "We can provide a short checklist of documents needed for your matter — tell us which service (e.g., conveyancing, probate, employment).";

      // fees and timelines
      if (text.includes("fees") || text.includes("how much") || text.includes("cost")) return "Fees depend on the matter's complexity. Provide a brief summary and we'll suggest a fee estimate or arrange a fixed-fee consult.";

      if (text.includes("thank")) return "You are welcome — happy to help. Would you like to book an appointment or speak to an advocate?";

      return "Our legal team can assist — please share a few details or say 'book' to request an appointment or 'call' to speak to an advocate.";
    };
    ...
    form.addEventListener("submit", (e) => {
      ...
      window.setTimeout(() => {
        ...
        if (typeof reply === "string") addMessage(reply);
        else if (reply && reply.type === "openModal") {
          addMessage("I'll open the appointment form for you.");
          openAppointmentModal(reply.practice);
        } else if (reply && reply.type === "call") {
          addMessage(`Opening phone dialer: ${reply.phone}`);
          window.location.href = `tel:${reply.phone.replace(/\s+/g, "")}`;
        } else if (reply && reply.type === "openUrl") {
          addMessage("Opening WhatsApp...");
          window.open(reply.url, "_blank");
        } else if (reply) addMessage(typeof reply === "string" ? reply : JSON.stringify(reply));
      }, 250);
    });
