/* ==========================================================================
   Content loader — pulls "Team", "Insights" and "Client Logos" from the
   Firebase Realtime Database and syncs them into the site's existing
   TEAM / INSIGHTS / CLIENTS arrays (data.js).
...
(function () {
  ...
  function loadClients() {
    return fbDb.ref("clients").once("value").then((snap) => {
      if (!snap.exists() || typeof CLIENTS === "undefined") return;
      const items = [];
      snap.forEach((child) => { items.push(child.val()); });
      items.sort(byOrder);
      CLIENTS.length = 0;
      items.forEach((item) => CLIENTS.push(item));
    }).catch((err) => { console.error("content-loader: clients fetch failed", err); });
  }

  Promise.all([loadTeam(), loadInsights(), loadClients()]).then(() => {
